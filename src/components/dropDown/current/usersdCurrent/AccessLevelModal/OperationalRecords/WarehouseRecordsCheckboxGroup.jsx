...
const WarehouseRecordsCheckboxGroup = ({ checkedState, setCheckedState, checkParentStatus }) => {
  const childCheckboxes = [
    ...
    const handleParentCheck = () => {
      const newState = !checkedState.warehouseRocords;
  
      // Set all first stage checkboxes based on the new state
      setCheckedState((prevState) => {
        const newCheckedState = { ...prevState, warehouseRocords: newState };
    ...
    const handleChildCheck = (child) => {
      setCheckedState((prevState) => {
        ...
        newCheckedState.warehouseRocords = !allUnchecked;
    ...
    <>
      <div className="form-check d-flex justify-content-start align-items-center ms-4 mt-2 mx-2">
        ...
        <input
          ...
          type="checkbox"
          id="warehouseRocords"
          checked={checkedState.warehouseRocords}
          onChange={handleParentCheck}
        />
        <label className="form-check-label me-2" htmlFor="warehouseRocords" style={{ textDecoration: "underline" }}>
          انبار
        </label>
      </div>

      {/* زیرمجموعه‌ها */}
      {checkedState.warehouseRocords && (
...
export default WarehouseRecordsCheckboxGroup;
